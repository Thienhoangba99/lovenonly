...
function check_login($email,$password){
		...
		if ($email == "" || $password =="") {
			echo "email or password is space!";
				...
				if($query->rowCount() > 0) {
				 ...
				 header('Location: http://localhost/thienhbgcd17084_source_code/App_shop/home1.php');
				} else {
	...
	$db=getDB();
		$query = "SELECT * FROM admin(adminemail,adminpassword)VALUES(?,?)";
		try{
				$sql = "SELECT adminemail, adminpassword FROM admin 
				...
				if($query->rowCount() > 0) {
				  $_SESSION['email'] = $email;
				 /*header("location:database_shop.php");*/
				 header('Location: http://localhost/thienhbgcd17084_source_code/App_shop/home2.php');
				}else {
				  echo "Username/Password is wrong";
				}
			}	
